...
import static dev.jibovico.apps.sharet.provider.ExternalStorageProvider.ROOT_ID_SECONDARY;
...
public class SAFManager {
    ...
    public static String getRootUri(Uri uri) {
        if (isTreeUri(uri)) {
            return dev.jibovico.apps.sharet.model.DocumentsContract.getTreeDocumentId(uri);
        }
        return dev.jibovico.apps.sharet.model.DocumentsContract.getDocumentId(uri);
    }

    private static Uri buildDocumentUriMaybeUsingTree(Uri uri, String docId) {
        return dev.jibovico.apps.sharet.model.DocumentsContract.buildDocumentUriMaybeUsingTree(uri, docId);
    }

    private static boolean isTreeUri(Uri uri) {
        return dev.jibovico.apps.sharet.model.DocumentsContract.isTreeUri(uri);
    }
