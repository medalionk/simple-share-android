...
import static dev.jibovico.apps.sharem.provider.ExternalStorageProvider.ROOT_ID_SECONDARY;
...
public class SAFManager {
    ...
    public static String getRootUri(Uri uri) {
        if (isTreeUri(uri)) {
            return dev.jibovico.apps.sharem.model.DocumentsContract.getTreeDocumentId(uri);
        }
        return dev.jibovico.apps.sharem.model.DocumentsContract.getDocumentId(uri);
    }

    private static Uri buildDocumentUriMaybeUsingTree(Uri uri, String docId) {
        return dev.jibovico.apps.sharem.model.DocumentsContract.buildDocumentUriMaybeUsingTree(uri, docId);
    }

    private static boolean isTreeUri(Uri uri) {
        return dev.jibovico.apps.sharem.model.DocumentsContract.isTreeUri(uri);
    }
